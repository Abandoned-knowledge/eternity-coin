...
async function submitForm(event: Event) {
  ...
  const password = (form.querySelector('input[name="input_enter_the_password"]') as HTMLInputElement).value;
...
<template>
  <form @submit.prevent="submitForm($event)" class="frame frame_chart flex h-fit w-[50%] flex-col items-center gap-5">
    ...
    <Input type="email" label="Email" placeholder="Enter the email" :required="true"  />
    <Input type="password" label="password" placeholder="Enter the password" :required="true" />
